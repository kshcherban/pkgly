...
- Optional SSO support is exposed through `/api/user/sso/login`. When enabled (`SecuritySettings.sso`), Pkgly expects the upstream SSO proxy to forward identity headers (`X-Forwarded-User` by default) and can auto-provision users when `auto_create_users` is true and the identity provider asserts `email_verified: true` in the JWT. Administrators can update these settings at runtime via `/api/security/sso` (exposed in the Admin UI) and the values persist in the `application_settings` table.
