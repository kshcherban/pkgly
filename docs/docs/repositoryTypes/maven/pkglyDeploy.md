# Maven Repository Deploy Format
To unlock all the features of pkgly we have a custom deploy mechanism. This system lets the pkgly take control of the data being passed. This is an optional feature and is not required.
...
## Notes
- All requests should contain the header `x-pkgly-deploy: maven 1`
...
## Publishing
So currently all the files are stored but not available to the public. They are not passed to the storage and are stored with the pkgly server

[POST] `{BASE_PATH}/deploy/{deployId}/publish`

This will tell pkgly to push all results to the storage.
...
```
## Yanking
The header of `x-pkgly-deploy` is not required for this request
