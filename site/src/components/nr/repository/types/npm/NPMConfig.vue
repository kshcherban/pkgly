...
import { defaultProxy, defaultVirtual, type NPMConfigType } from "./npm";
...
const npmRepositories = computed(() =>
  repositories.value.filter(
    (repo) => repo.repository_type.toLowerCase() === "npm" && repo.id !== props.repository,
  ),
);
...
const memberOptions = computed(() =>
  npmRepositories.value.map((repo) => ({
    value: repo.id,
...
function syncMemberNames() {
  const repoMap = new Map(npmRepositories.value.map((repo) => [repo.id, repo]));
...
async function load() {
  ...
  try {
    // Prefer virtual members API for existing repositories to hydrate DB state
    const membersResponse = await http.get(
      `/api/repository/${props.repository}/virtual/members`,
    );
...
function makeVirtualConfig(config?: import("./npm").NpmVirtualConfigType): import("./npm").NpmVirtualConfigType {
  ...
  }) as import("./npm").NpmVirtualConfigType;
}
</script>
